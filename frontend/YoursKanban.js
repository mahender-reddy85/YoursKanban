...
async function duplicateTask(id) {
    ...
    try {
        ...
        if (!taskToDuplicate) {
            console.error('Task not found for duplication. ID:', id);
            showToast('Task not found', 'error');
            return;
        }

        button = document.querySelector(`.duplicate-btn[data-id="${id}"]`);
        ...
        newTask.title = (taskToDuplicate.title || '').includes(' (Copy)') ? taskToDuplicate.title : `${taskToDuplicate.title || 'Untitled'} (Copy)`;
        ...
        if (newTask.dueDate) {
            ...
            if (isNaN(d.getTime()) || d.getFullYear() === 1970 || d.getTime() <= 0) {
        ...
        let createdTask;
        if (response && response.data) createdTask = response.data;
        else if (response && response.id) createdTask = response;
        else if (response && typeof response === 'object') createdTask = response;
        else throw new Error('Invalid response from server');

        // Ensure dates preserved if backend returns invalid ones
        const originalDueDate = validatedTask.dueDate;
        if (!createdTask.dueDate && originalDueDate) createdTask.dueDate = originalDueDate;
        if (!createdTask.due_date && originalDueDate) createdTask.due_date = originalDueDate;
