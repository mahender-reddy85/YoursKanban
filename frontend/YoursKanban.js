...
const Auth = (() => {
    ...
    const hashPassword = (password) => {
        // Simple hashing - in a real app, use a proper hashing library
        return btoa(password);
    };

    const getUsers = () => {
        try {
            return JSON.parse(localStorage.getItem('kanbanflow_users')) || [];
        } catch (e) {
    ...
    const saveUsers = (users) => {
        try {
            localStorage.setItem('kanbanflow_users', JSON.stringify(users));
            return true;
        } catch (e) {
    ...
    return {
        ...
        register: (name, email, password) => {
            ...
            if (success) {
                ...
                localStorage.setItem('kanbanflow_auth', JSON.stringify({
                    userId: newUser.id,
                    email: newUser.email,
                    name: newUser.name
                }));
        ...
        // Logout user
        logout: () => {
            localStorage.removeItem('kanbanflow_auth');
            return { success: true };
        },
        ...
        getCurrentUser: () => {
            try {
                return JSON.parse(localStorage.getItem('kanbanflow_auth'));
            } catch (e) {
                return null;
            }
        },
...
// Update the init function to check authentication
...
// Add these new functions for auth UI management
...
function handleAuthFormSubmit(e) {
    ...
    if (result.success) {
        ...
        hideAuthModal();
        updateUIForAuthenticatedUser(result.user || Auth.getCurrentUser());
        // Refresh the board to show user-specific data
...
function setupAuthEventListeners() {
    ...
    document.getElementById('toggleToSignup')?.addEventListener('click', () => showAuthModal('signup'));
    document.getElementById('toggleToLogin')?.addEventListener('click', () => showAuthModal('login'));
...
// Update loadState and saveState to be user-specific
...
window.loadState = function() {
    ...
    try {
        const savedState = localStorage.getItem(`kanbanflow_state_${currentUser.userId}`);
        if (savedState) {
            const parsedState = JSON.parse(savedState);
            state.tasks = Array.isArray(parsedState.tasks) ? parsedState.tasks : [];
            state.lastDeletedTask = parsedState.lastDeletedTask || null;
...
window.saveState = function() {
    ...
    try {
        const stateToSave = {
            tasks: state.tasks || [],
            lastDeletedTask: state.lastDeletedTask || null
        };
        localStorage.setItem(`kanbanflow_state_${currentUser.userId}`, JSON.stringify(stateToSave));
    } catch (error) {
